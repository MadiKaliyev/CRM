...
def save_to_docs(company_name, people_data, chislo_chelovek):
    if not company_name:
        messagebox.showerror("Ошибка", "Пожалуйста, введите название компании.")
        return

    # Открытие документа ПРОМ.БЕЗ-НОМЕРА.docx
    file_path_1 = r"C:\Users\Madi\Desktop\ПРОГРАММА\Программа\НОМЕРА\ПРОМ.БЕЗ-НОМЕРА.docx"
    try:
        doc1 = Document(file_path_1)
    except Exception as e:
        messagebox.showerror("Ошибка", f"Ошибка при открытии файла ПРОМ.БЕЗ-НОМЕРА.docx: {e}")
        return
    ...
    # Открытие документа ПротоколПРОМ.docx
    file_path_2 = r"C:\Users\Madi\Desktop\ПРОГРАММА\Программа\файлы\ПротоколПРОМ.docx"
    try:
        doc2 = Document(file_path_2)
    except Exception as e:
        messagebox.showerror("Ошибка", f"Ошибка при открытии файла ПротоколПРОМ.docx: {e}")
        return
    ...
    for paragraph in doc2.paragraphs:
        ...
        replace_text(paragraph, 'ДАТА', current_date)
        replace_text(paragraph, 'НАИМЕНОВАНИЕ', company_name)
    ...
    for fio, position, education, otvetstvennost in people_data:
        found_empty_row = False
        for row in table2.rows:
            if all(cell.text == '' for cell in row.cells):
                found_empty_row = True
                ...
                set_cell_text(row.cells[3], education)
        ...
        if not found_empty_row:
            ...
            set_cell_text(new_row.cells[3], education)
            set_cell_text(new_row.cells[4], "Сдал")
            max_num_2 += 1

    # Создание основной директории sanitized_company_name
    ...
    # Внутри уникальной поддиректории создаем поддиректорию "Пром.Без"
    sub_directory = os.path.join(main_directory, "Пром.Без")
    sub_counter = 1
    while os.path.exists(sub_directory):
        sub_directory = os.path.join(main_directory, f"Пром.Без({sub_counter})")
        sub_counter += 1
    os.makedirs(sub_directory)

    # Сохранение doc2 в поддиректории
    doc2.save(os.path.join(sub_directory, "Протокол.docx"))

    # Открытие документа ШАБЛОН.docx
    file_path_template = r"C:\Users\Madi\Desktop\ПРОГРАММА\Программа\файлы\ШАБЛОН.docx"
    try:
        doc_template = Document(file_path_template)
    except Exception as e:
        messagebox.showerror("Ошибка", f"Ошибка при открытии файла ШАБЛОН.docx: {e}")
        return
    ...
    current_month_russian = russian_months[current_month]
        ...
        for row in table.rows:
            for cell in row.cells:
                for paragraph in cell.paragraphs:
                    ...
                    replace_text(paragraph, 'ОТВЕТСТВЕННОСТЬ', people_data[i][3])
                    replace_text(paragraph, 'ЧИСЛО', f'{current_day} {current_month_kazakh}')

                    if i < int(chislo_chelovek):
                        replace_text(paragraph, 'датаказ', f"{current_day} {current_month_kazakh2} {current_year_over}")
                        replace_text(paragraph, 'ДАДАКОНЕЦ', f"{current_day} {current_month_russian} {current_year_over}")
                        replace_text(paragraph, 'ЧАС', str(10))
                    else:
                        replace_text(paragraph, 'датаказ', f"{current_day} {current_month_kazakh2} {current_year_over3}")
                        replace_text(paragraph, 'ДАДАКОНЕЦ', f"{current_day} {current_month_russian} {current_year_over3}")
                        replace_text(paragraph, 'ЧАС', str(40))

                    replace_text(paragraph, 'МЕСЯЦ', current_month_russian)
                    replace_text(paragraph, 'ГОД', str(current_year))
                    replace_text(paragraph, 'ДАТА', f"{current_day} {current_month_russian} {current_year}")
                    replace_text(paragraph, 'НОМЕР', str(num_protocol))
        start_range += 1

    # Сохранение шаблона в поддиректории "Пож.Без"
